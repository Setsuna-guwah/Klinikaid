...
- **SO-D (RBAC Enforced):** Base middleware routing and SQL RLS policies enforce access limits based on user role and department.
- **SO-E (ISO 25010 Quality):** Strict types, build tests, logging, and error wrappers provide funcional suitability, reliability, and security metrics from day one.

---

## 5. Assumptions & Modifications

- **Font Selection:** Replaced Next.js 15 Geist local font imports with `next/font/google` Inter font to avoid webpack loading errors in Next.js 14.
- **Dynamic Renderers:** Forced dynamic rendering (`export const dynamic = "force-dynamic"`) on `/` and `/403` to prevent Next.js from throwing static compilation warnings when using cookie-reading auth checks.
- **Embedding Dimensions:** Schema updated for 768-dimension vectors (`vector(768)`) in Supabase PostgreSQL to accommodate the native Gemini embedding model.

---

## 6. Actions Required from User

1. **Populate Credentials:** Edit `.env.local` and replace the placeholder values with your real Supabase URLs, Anon Key, Service Role Key, and Gemini API Key.
2. **Execute Migrations:** Open the SQL Editor in your Supabase Dashboard, copy the entire content of [schema.sql](file:///c:/Users/johnr/Desktop/Klinikaid/src/lib/db/schema.sql), and run it. This will create all tables, indexes, triggers, and RLS policies.
